...
ai.post("/mindmap/:workspaceId", async (c) => {
  ...
  // Enforce daily AI request limit
  if (!(await UsageService.canMakeAIRequest(user.id))) {
    return c.json({ error: "Daily AI request limit reached (10/day). Please try again tomorrow." }, 403);
  }
